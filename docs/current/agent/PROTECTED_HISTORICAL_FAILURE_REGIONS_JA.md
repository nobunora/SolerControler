...
| `app/energy_plan/soc_projection.py`、`app/energy_plan/soc_cost.py::simulate_daytime_soc_replay`、`static/dashboard.js` の時間別SOC表示 | 2026-09-05 ローカル画面で計画SOC欠落時に予想SOC系列が全点null。その後2026-09-18に、欠損を埋めるためDashboardが独自SOCモデルを持つとEnergy Planの決定モデルと乖離することを確認 | Dashboard独自再計算で、同じ計画から異なるSOC系列が表示される。device `SocChargeMode=50` を連続計画SOCとして誤解する | 2026-09-18以降はEnergy Planを唯一の時間別SOC予測主体とする。03時実測SOCをanchorに、planner-owned charge scheduleとoptimizer共通daytime replayから `forecast_soc_percent` / `forecast_grid_charge_kwh` を保存する。Dashboardは保存値と過去実績を表示するだけで、欠損時にSOCや夜間系統充電を再計算しない。reconstructed historyへ後日のSOC予測を付与しない。device `soc_charge_mode` と `planned_target_soc_percent` を混同しない。回帰契約は `tests/test_soc_single_source_contract.py` と `tests/test_dashboard_forecast_soc_contract.py` で固定する。 |
...
| `app/runtime/cloud_job.py::_RunnerMonitorDevicePort.read_soc`、`_monitor_partial_forced_and_stop` | 2026-09-06 設定SOC 100%に対して実績65%で停止 | 遅延CSVが03制御へ混入すること、またはKP-NET画面SOCの単発取得失敗で強制充電を早期終了すること | 03制御SOCはKP-NET画面の直接値だけを使い、CSVへフォールバックしない。監視中の取得失敗は連続回数で数え、正常取得でリセットし、`max_consecutive_soc_failures` 到達時だけstandbyへ遷移する。06:45/06:50/06:55の時刻所有権は維持する。 |
