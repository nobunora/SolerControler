...
| `app/energy_plan/soc_projection.py::simulate_night_charge_soc` / `simulate_daytime_soc`、`static/dashboard_calculations.js::forecastSocValues` | 2026-09-05 ローカル画面で計画SOC欠落時に予想SOC系列が全点null。その後2026-09-18にSOC予測の所有権をEnergy Planへ移管 | 保存計画欠落を理由にDashboardが独自のSOC式を持つと、Plannerと異なる予測を表示し、機器候補SOCを目標SOCと誤認する | 時間別SOC予測はEnergy Planが唯一生成し、`forecast_soc_percent`として保存する。Dashboardは保存値と同時間帯の実測SOCを表示上選択するだけで、目標SOC・PV/負荷・現在SOCから未来SOCを再計算しない。正規予測が欠ける未来時刻はnullのままとし、後日の履歴再構築でもSOC予測を捏造しない。 |
