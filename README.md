...
- `KP_SETTINGS_SEQUENCE=forced-only` : 夜間経済モードのみ（既定）
- `KP_SETTINGS_SEQUENCE=forced-then-green` : 夜間経済モード適用後にグリーンモードも適用
- `KP_DYNAMIC_FORCED_PROFILE=true` : `artifacts/night_charge_plan.json` から
  - 必要充電量(kWh)
  - 7時目標SOC
  - 夜間実測充電レート(kW推定)
  を使って、夜間経済モードの `SOC下限` / `SOC上限` / `充電開始時刻` を自動算出
- `KP_DYNAMIC_MODE_SWITCH_BY_TIME=true` : 現在時刻で設定先を自動選択
  - 夜間(23:00-07:00): 経済モード + SOC下限(安心)=最大値
...
# 3) 設定実登録（時刻で自動切替: 夜間=経済モード / 日中=グリーン）
...
- `csv/*.csv`
- `kpi_plot.png`
- `confirm_night-economy.html`（`KP_SETTINGS_SEQUENCE=forced-only`）
- `confirm_night-economy.html`, `confirm_green-mode.html`（`KP_SETTINGS_SEQUENCE=forced-then-green`）
- `kpnet_summary.json`
