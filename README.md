...
- `NIGHT_RESERVE_SOC_PERCENT=0`（固定の翌朝SOC目標下限を設けず、料金最適化に委ねる）
...
- `LOAD_COMFORT_MODEL_ENABLED=true`（固定温湿度しきい値を使わない適応快適・熱慣性HGBで時間別消費を予測）
...
- `ADJUST03_MIN_TARGET_SOC_PERCENT=0`（03:00制御の固定SOC下限を設けない）
...
- 履歴は23時スロットで1日1回全再構築し、03/07・予報更新ではbootstrapだけ更新する。後からhistoryを読み込んでも最新bootstrapと重なる日付は上書きしない。31日より古い履歴を訂正・バックフィルした直後は `DASHBOARD_SNAPSHOT_FULL_HISTORY_REBUILD=true` で即時全再構築できる
