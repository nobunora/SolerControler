...
def build_comfort_feature_map(
    ts: datetime,
    weather: dict[datetime, dict[str, float]],
) -> dict[str, float] | None:
    """Build the stable feature values used by the comfort-load model.

    Analysis tools use this function to inspect the same input schema as the
    production model, so their weather-vector results remain comparable.
    """
    ...
    *,
    ...
    for day in sorted(actual_history):
        ...
        for hour, values in sorted(actual_history[day].items()):
            ...
            features = build_comfort_feature_map(ts, weather)
    ...
    target_features = [build_comfort_feature_map(ts, weather) for ts in target_times]
